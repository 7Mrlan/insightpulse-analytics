 ...
 * 校验权限函数
 */
function hasPermission(requiredPermission: Api.Auth.Permission | Api.Auth.Permission[]): boolean {
...
export function setupRouterGuard(router: Router) {
  router.beforeEach(async to => {
    ...
    if (permission && !(typeof permission === 'string' || Array.isArray(permission))) {
      logError(`路由 ${to.fullPath} 的 permission 配置错误，需为 string 或 string[]`);
      return false;
    }
    ...
    // **检查权限**
    if (permission && !hasPermission(permission as Api.Auth.Permission | Api.Auth.Permission[])) {
      return { name: '403' };
    }
